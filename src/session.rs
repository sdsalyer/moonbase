...
use crate::menu::{Menu, MenuAction, MenuData, MenuRender, CurrentMenu, Display};
...
impl BbsSession {
    ...
    fn menu_handle_action(&mut self, stream: &mut TcpStream, action: MenuAction) -> BbsResult<bool> {
        match action {
            ...
            MenuAction::Logout => {
                self.username = None;
                self.show_message_with_stream(stream, "SYSTEM MESSAGE", "You have been logged out.", Some(Color::Yellow))?;
                Ok(true)
            },
            MenuAction::Quit => {
            ...
            MenuAction::ShowMessage(message) => {
                self.show_message_with_stream(stream, "SYSTEM MESSAGE", &message, Some(Color::Yellow))?;
                Ok(true)
            },
        }
    }
    ...
    fn handle_login(&mut self, stream: &mut TcpStream) -> BbsResult<()> {
        ...
        let instructions = format!("Max username length: {} characters\n\nEnter username (or press Enter to cancel):", 
            self.config.features.max_username_length);
        
        self.box_renderer.render_message_box(stream, "USER LOGIN", &instructions, width, Some(Color::Cyan))?;
        
        let username = self.get_input(stream, "\nUsername: ")?;
        
        if username.is_empty() {
            self.show_message_with_stream(stream, "LOGIN", "Login cancelled.", Some(Color::Yellow))?;
            return Ok(());
        }
        
        if username.len() > self.config.features.max_username_length {
            let msg = format!("Username too long (max {} characters)", 
                self.config.features.max_username_length);
    ...
    fn force_login(&mut self, stream: &mut TcpStream) -> BbsResult<()> {
        ...
        self.box_renderer.render_message_box(stream, "LOGIN REQUIRED", message, width, Some(Color::Yellow))?;
        ...
        if self.username.is_none() {
            return Err(BbsError::AuthenticationFailed("Too many failed login attempts".to_string()));
        }
    ...
    fn attempt_login(&mut self, stream: &mut TcpStream) -> BbsResult<bool> {
        ...
        let username = self.get_input(stream, &format!("Login attempt {}/3\nUsername: ", self.login_attempts))?;
        
        if username.len() > self.config.features.max_username_length {
            stream.queue(SetForegroundColor(Color::Red))?;
            stream.queue(Print(&format!("Username too long (max {} characters)\n\n", 
                self.config.features.max_username_length)))?;
            stream.queue(ResetColor)?;
            stream.flush()?;
            return Ok(true);
    ...
    fn menu_show(&self, stream: &mut TcpStream, render: &MenuRender) -> BbsResult<()> {
        ...
        stream.queue(Clear(ClearType::All))?;
        stream.queue(cursor::MoveTo(0, 0))?;
        self.box_renderer.render_menu(stream, &render.title, &render.items, width, None)?;
        Ok(())
    }
    
    /// Display a message box with stream
    fn show_message_with_stream(&mut self, stream: &mut TcpStream, title: &str, message: &str, color: Option<Color>) -> BbsResult<()> {
        let width = self.config.ui.menu_width + 20;
        
        stream.queue(Clear(ClearType::All))?;
        stream.queue(cursor::MoveTo(0, 0))?;
        
        self.box_renderer.render_message_box(stream, title, message, width, color)?;
    ...
    fn show_goodbye(&mut self, stream: &mut TcpStream) -> BbsResult<()> {
        ...
        let goodbye_msg = format!("Thanks for visiting {}!\n\nSysOp: {}\n\n🚪 Come back anytime! 🚪", 
            self.config.bbs.name, self.config.bbs.sysop_name);
        
        self.box_renderer.render_message_box(stream, "GOODBYE", &goodbye_msg, width, Some(Color::Magenta))?;
    ...
    fn show_feature_disabled(&mut self, stream: &mut TcpStream, feature_name: &str) -> BbsResult<()> {
        let width = self.config.ui.menu_width + 20;
        let message = format!("⚠️  {} has been disabled by the SysOp.\n\nContact {} for more information.", 
            feature_name, self.config.bbs.sysop_name);
        
        stream.queue(Clear(ClearType::All))?;
        stream.queue(cursor::MoveTo(0, 0))?;
        self.box_renderer.render_message_box(stream, "FEATURE DISABLED", &message, width, Some(Color::Red))?;
