...
//     fn post_bulletin(&mut self, request: &BulletinRequest, config: &crate::config::BbsConfig) -> BbsResult<u32>;
...
//         }
//
//         let content = fs::read_to_string(&self.bulletins_file)
//             .map_err(|e| BbsError::Configuration(format!("Failed to read bulletins file: {}", e)))?;
//
//         if content.trim().is_empty() {
//             return Ok(()); // Empty file is OK
//         }
...
//             .map_err(|e| BbsError::Configuration(format!("Failed to parse bulletins file: {}", e)))?;
//
//         // Load bulletins
//         if let Some(bulletins_obj) = data.get("bulletins").and_then(|v| v.as_object()) {
//             for (id_str, bulletin_data) in bulletins_obj {
...
//                 })?;
//
//                 let bulletin: Bulletin = serde_json::from_value(bulletin_data.clone())
//                     .map_err(|e| BbsError::Configuration(format!("Failed to parse bulletin: {}", e)))?;
//
//                 self.bulletins_cache.insert(id, bulletin);
//                 if id >= self.next_id {
//                     self.next_id = id + 1;
...
//             .map_err(|e| BbsError::Configuration(format!("Failed to serialize bulletins: {}", e)))?;
//
//         fs::write(&self.bulletins_file, content)
//             .map_err(|e| BbsError::Configuration(format!("Failed to write bulletins file: {}", e)))?;
//
//         Ok(())
//     }
//
...
//         );
//
//         self.bulletins_cache.insert(id, bulletin.clone());
//         self.save_all_bulletins(&self.bulletins_cache)?;
//
//         Ok(id)
//     }
...
//             Ok(())
//         } else {
//             Err(BbsError::InvalidInput(format!("Bulletin {} not found", bulletin_id)))
//         }
//     }
//
...
//     fn get_unread_bulletins(&self, username: &str) -> BbsResult<Vec<Bulletin>> {
//         let bulletins: Vec<Bulletin> = self.bulletins_cache
//             .values()
//             .filter(|b| !b.is_read_by(username))
//             .cloned()
//             .collect();
