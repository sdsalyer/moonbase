use super::{Menu, MenuAction, MenuData, MenuRender, CurrentMenu};
    ...
    fn render(&self, data: MenuData) -> MenuRender {
        if !data.config.features.bulletins_enabled {
            let items = vec![
                ...
                MenuItem::info(&format!("Contact {} for more information.", data.config.bbs.sysop_name)),
                MenuItem::separator(),
        ...
        let welcome_msg = format!("• Welcome to {}! - Posted by {}", 
        ...
        if data.is_logged_in() || data.allow_anonymous() {
            ...
            items.push(MenuItem::disabled_option("P", "Post new bulletin (login required)"));
        }
    ...
    fn handle_input(&self, data: MenuData, input: &str) -> MenuAction {
        if !data.config.features.bulletins_enabled {
            return match input.to_lowercase().as_str() {
                ...
                _ => MenuAction::ShowMessage("Bulletin Board is disabled. Press B to go back.".to_string()),
            };
        }
        ...
        match input.to_lowercase().as_str() {
            ...
            "p" => {
                if data.is_logged_in() || data.allow_anonymous() {
                    MenuAction::ShowMessage("Posting bulletin... (Feature coming soon!)".to_string())
                } else {
